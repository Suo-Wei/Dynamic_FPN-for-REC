...
class SpatialGate(nn.Module):
    ...
    def init_parameters(self, init_gate=0.99):
        ...
        # nn.init.normal_(self.gate_conv.weight, std=1)
    ...
    def forward(self, data_input, gate_input, masked_func=None,lang=False):
        ...
        if lang:
            ...
            gate = self.gate_activate(data_input.permute(0,2,3,1) @ gate_input).permute(0,3,1,2)
            return data_input
        else:
            gate = self.gate_activate(self.gate_conv(gate_input))
...
class DynamicConv2D(nn.Module):
    ...
    def forward(self, input,lang_input = False):
        ...
        if lang_input:
            ...
            output = self.gate(data,lang.squeeze(),lang=lang_input)
        else:
...
class DynamicScale(nn.Module):
    ...
    def lang_att(self,lang,mask,l):
        lang = self.lang_fc[l](lang.permute(1, 2, 0))
        lang_att_list = []
        att_list = []
    ...
    def forward(self, inputs):
        ...
        for l, x in enumerate(inputs[0]):
            if inputs[1] is not None:
                lang,lang_att_list,att_list = self.lang_att(inputs[1],inputs[2],l)
                inputs[-1].append(att_list)
        ...
        return [outputs,lang,inputs[2],inputs[-1]]

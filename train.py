...
def get_args_parser():
    ...
    parser.add_argument('--detr_model', default='checkpoints/detr-r101-gref_new.pth', type=str, help='detr model')
    parser.add_argument('--bert_model', default='saved_models', type=str, help='bert model')
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        log_stats = {**{'train_{k}': v for k, v in train_stats.items()},
                     **{'validation_{k}': v for k, v in val_stats.items()},
                     'epoch': epoch,
        ...
        if args.output_dir:
            ...
            if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % 10 == 0:
                checkpoint_paths.append(output_dir / 'checkpoint{epoch:04}.pth')

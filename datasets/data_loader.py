...
class TransVGDataset(data.Dataset):
    ...
    def pull_item(self, idx):
        ...
        if not (self.dataset == 'referit' or self.dataset == 'flickr'):
            ...
            bbox = np.array(bbox, dtype=int)
        if self.dataset == 'unc' or self.dataset=='gref' or self.dataset=='gref_umd' or self.dataset=='unc+':
            img_file = img_file[-31:]
            self.im_dir = '/home/B/suowei/data/coco2014/train2014/'
        elif self.dataset == 'referit':
            img_file_ = int(img_file.split('.')[0])
            img = img_file_ // 1000
            self.im_dir = '/home/suowei/saiapr_tc-12/'
            if img < 10:
                img = '0'+str(img)
                self.im_dir = self.im_dir+img+'/images'
            else:
                img = str(img)
                self.im_dir = self.im_dir + img+'/images'
        img_path = osp.join(self.im_dir, img_file)
